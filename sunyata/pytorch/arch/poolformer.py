...
class PoolformerCfg(BaseCfg):
    ...
    drop_rate: float = 0.  # drop rate

    type: str = 'standard'  # standard iter iter_attn


def poolformer(cfg:PoolformerCfg):
    arch_settings ={
        'tiny': {
            'depths': [2, 2, 6, 2],
    ...
    model = PoolFormer(layers=layers, embed_dims=embed_dims,
...
class PlPoolFormer(ClassifierModule):
    def __init__(self, cfg: PoolformerCfg):
        super(PlPoolFormer, self).__init__(cfg)
        self.model = poolformer(cfg)
