...
class ConvNeXtV1(BaseModule):
    def __init__(self, cfg: ConvMixerCfg):
        ...
        dp_rates=[x.item() for x in torch.linspace(0, cfg.drop_rate, sum(depths))] 
        cur = 0
        for i in range(4):
            stage = nn.Sequential(
        ...
        self.head = nn.Linear(dims[-1], cfg.num_classes) # classifier head
